...
- **UNSAFE (CRASH):** Declaring a `@State` (for `@Observable`) or `@StateObject` (for Combine) property in the root `App` struct if its initializer touches Firebase. Property initializers run _before_ the `App.init()` body, meaning the object's `init()` will fire before Firebase is configured.
