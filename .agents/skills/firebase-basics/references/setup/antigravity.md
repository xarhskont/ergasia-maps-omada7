...
3. **Install Skills**: If both checks fail, run the following command to install the Firebase agent skills:
    ...
    ```
    _Note: Omit `--yes` and `--global` to choose the installation location manually. If prompted interactively in the terminal, ensure you send the appropriate user choices via standard input to complete the installation._
...
1. **Locate `mcp_config.json`**: Find the configuration file for your operating system:
    ...
    _Note: If the `.gemini/antigravity/` directory or `mcp_config.json` file does not exist, create them and initialize the file with `{ "mcpServers": {} }` before proceeding._
...
3. **Add or Update Configuration**: If the `firebase` block is missing or incorrect, add it to the `mcpServers` object:
    ...
    ```
    _CRITICAL: Merge this configuration into the existing `mcp_config.json` file. You MUST preserve any other existing servers inside the `mcpServers` object._
