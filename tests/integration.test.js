...
jest.mock('https://www.gstatic.com/firebasejs/9.23.0/firebase-firestore.js', () => ({
    collection: jest.fn(),
    addDoc: jest.fn(),
    query: jest.fn(),
    where: jest.fn(),
...
describe('Rating System Integration Tests', () => {
    ...
    test('should block rating if user is not authenticated', async () => {
        ...
        await expect(submitRating('job_abc', 'user_xyz', 5, 'Great!', 'employer_to_freelancer'))
    ...
    test('should allow rating if no previous review exists', async () => {
        ...
        const result = await submitRating('job_abc', 'user_xyz', 5, 'Great!', 'employer_to_freelancer');
